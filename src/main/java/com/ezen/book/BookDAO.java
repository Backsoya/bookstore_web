...
import com.ezen.utill.JDBCUtil;
...
public class BookDAO {
	...
	String sql = "select bno,title,writer,price,publisher,regdate "
		+ "from booktbl where disp='y' order by regdate desc";
	try {
	...
	String sql = "select count(*) from booktbl where " + searchtype + " like ? and  disp='y' ";
	...
	String sql = "insert into booktbl(bno,title,writer,price,publisher,content"
		+ ",srcFilename,saveFilename,savePath) " + "values(book_seq.nextval,?,?,?,?,?,?,?,?)";
	...
	ResultSet rs = null;
	try {
	    pstmt = conn.prepareStatement("select * from booktbl where bno=? and disp='y' ");
	    pstmt.setInt(1, bno);
	    rs = pstmt.executeQuery();
	    if (rs.next()) {
		vo = new BookVO(rs.getInt("bno"), rs.getString("title"), rs.getString("writer"), rs.getInt("price"),
			rs.getString("publisher"), rs.getDate("regdate"), rs.getString("content"),
			rs.getString("srcFilename"), rs.getString("saveFilename"), rs.getString("savePath"));
	    }
	} catch (SQLException e) {
	    e.printStackTrace();
	} finally {
	...
	if (oFileName == null)
	    sql = "update booktbl set title=?,writer=?" + ",publisher=?,price=?,content=? where bno=?";
	else
	    sql = "update booktbl set title=?,writer=?" + ",publisher=?,price=?,content=?,srcFilename=?"
	...
	PreparedStatement pstmt = null;
	try {
	    pstmt=conn.prepareStatement("update booktbl set disp='n' where bno=?");
	    pstmt.setInt(1, bno);
	    result = pstmt.executeUpdate();
	} catch (SQLException e) {
	    e.printStackTrace();
