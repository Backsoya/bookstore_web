...
<%@ page language="java" contentType="text/html; charset=UTF-8" pageEncoding="UTF-8" %>
	<%@ include file="../include/header.jsp" %>
		...
		<link rel="stylesheet" href="${pageContext.request.contextPath}/star-rating/css/star-rating.min.css" media="all"
			type="text/css" />
		<link rel="stylesheet" href="${pageContext.request.contextPath}/star-rating/themes/krajee-svg/theme.min.css"
			media="all" type="text/css" />
		<script src="${pageContext.request.contextPath}/star-rating/js/star-rating.min.js"
			type="text/javascript"></script>
		<script src="${pageContext.request.contextPath}/star-rating/themes/krajee-svg/theme.min.js"
			type="text/javascript"></script>
			...
			<form action="book?cmd=edit" method="post" enctype="multipart/form-data" id="uploadForm" name="uploadForm">
				...
				<table class="table table-sm table-bordered">
					...
					<tr>
						...
						<td class="edit" style="display: none;"><input type="text" size="120" maxlength="50"
								name="title" id="title" value="${vo.title}" placeholder="도서명입력" required></td>
					</tr>

					<tr>
						<th>저자</th>
						<td class="disp">${vo.writer}</td>
						<td class="edit" style="display: none;"><input type="text" size="120" maxlength="30"
								name="writer" value="${vo.writer}" id="writer" placeholder="저자명입력"></td>
					</tr>
					...
					<tr>
						...
						<td class="edit" style="display: none;"><input type="text" size="120" maxlength="30"
								name="publisher" value="${vo.publisher}" id="publisher" placeholder="출판사명입력"></td>
					</tr>
					...
					<tr>
						...
						<td class="edit" style="display: none;"><input type="text" size="120" maxlength="7" name="price"
								id="price" value="${vo.price}" onkeydown="inputNum(this)" placeholder="가격입력" required>
						</td>
					</tr>
					...
					<tr>
						<th>도서내용</th>
						<td class="disp">
							<% BookVO vo=(BookVO) request.getAttribute("vo"); if (vo !=null) { String
								content=vo.getContent(); if (content !=null) out.write(content.replaceAll("\r\n", "<br>"
								)); } %>
						</td>
						<td class="edit" style="display: none;"><textarea name="content" id="content" cols="119"
								rows="10" maxlength="1000">${vo.content}</textarea></td>
					</tr>
					...
					<tr>
						...
						<td class="disp">
							<c:if test="${vo.saveFilename!=null}">
								<img src="book?cmd=imgDown&upload=${vo.savePath}&saveFname=${vo.saveFilename}&originFname=${vo.srcFilename}"
									alt="" height="300px">
							</c:if>
						</td>
						...
						<td class="edit" style="display: none;">
							...
							<div class="form-group row">
								...
								<div class="col-sm-10">
									<input type="file" name="file" id="file"> <small class="text-muted">(파일크기 : 2MB /
										이미지 파일만 가능)</small> <small id="file" class="text-info"></small>
								</div>
							</div>
						</td>
					</tr>
					...
					<tr>
						<th>평균 별점</th>
						<td><input id="avgscore" name="avgscore" value="${avgScore }" class="rating rating-loading"
								data-size="sm" readonly="readonly"> &nbsp;&nbsp;&nbsp; 평가자 인원수 : <span id="cnt">${cnt
								}</span></td>
					</tr>
				...
				<c:if test="${sessionScope.mvo != null }">
					...
					<div style="align-content: center;">
						...
						<table>
							<tr>
								<td width="300px"><input id="score" name="score" class="rating rating-loading"
										data-min="0" data-max="5" data-step="0.5" data-size="sm" required="required">
								</td>
								<td>평가글 남기기 : <input type="text" maxlength="100" id="cmt" name="cmt" size="50"
										required="required">
				...
				<div class="btn_rud">
					<button type="button" id="btnList"
						onclick="location.href='book?cmd=list&page=${page}&searchword=${searchword}&searchtype=${searchtype}'"
						class="btn btn-success">도서목록</button>
					<c:if test="${sessionScope.mvo.grade == 'a'}">
						...
						<button type="reset" id="btnCancle" onclick="bookCancle()" class="btn btn-info"
