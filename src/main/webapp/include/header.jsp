	...
	<div class="btn_log">
		<c:choose>
			<c:when test="${sessionScope.mvo == null }">
				...
				<button type="button" id="logout" name="logout" onclick="" class="btn btn-primary">회원가입</button>
			</c:when>
			<c:otherwise>
			${sessionScope.mvo.name }님
				<button type="button" id="logout" name="logout" onclick="location.href='logout'" class="btn btn-warning">로그아웃</button>
			</c:otherwise>
		</c:choose>
